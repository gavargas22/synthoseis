...
class RandomHorizonStack(Horizons):
    ...
    #     # Make a dask array which will hold the futures for the depth maps
    ...
    def create_depth_maps(self):
        ...
        # Initialize Zarr store and root group
        zarr_store = zarr.DirectoryStore(self.cfg.zarr_filename)
        zarr_root = zarr.group(zarr_store, overwrite=True)

        # Create an initial Zarr array to store the depth maps
        depth_maps_shape = (1, *previous_depth_map.shape)
        depth_maps_zarr = zarr_root.create_dataset(
            "depth_maps",
            shape=depth_maps_shape,
            chunks=(1, *previous_depth_map.shape),

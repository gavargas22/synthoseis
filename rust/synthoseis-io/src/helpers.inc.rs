...
fn chunk_key(indices: &[usize]) -> String {
    indices
        .iter()
        .map(|i| i.to_string())
        .collect::<Vec<_>>()
...
fn write_chunk_bytes(array_dir: &Path, key: &str, bytes: &[u8]) -> Result<()> {
...
fn parse_usize3(v: &Value) -> Result<[usize; 3]> {
    let arr = v
        .as_array()
...
fn parse_dimensions(attrs: &Value) -> Result<[Dimension; 3]> {
    let dims = attrs
        .get("dimension")
...
fn update_root_attr_u64(root: &Path, key: &str, value: u64) -> Result<()> {
...
fn update_stats(root: &Path, samples: &[f32]) -> Result<()> {
    if samples.is_empty() {
        return Ok(());
    }

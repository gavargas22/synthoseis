#!/usr/bin/env python3
"""Regenerate compact parity cubes for CI (labels RLE + deterministic angles).
...
Angle stacks: angle[i,j,k] = 0.1*i + 0.05*j + 0.02*k (no RNG; synthesized in Rust).
Labels: run-length encoded as [value, count] pairs.
"""
...
def fit_plane_lsq(xyz: np.ndarray) -> list[float]:
    ...
    g[:, 0] = xyz[:, 0]
    g[:, 1] = xyz[:, 1]
    (a, b, c), _, _, _ = np.linalg.lstsq(g, xyz[:, 2], rcond=-1)
...
def main() -> None:
    ...
    out = {
        ...
        "rotate_point": {
            "cases": [
                {"x": 1.0, "y": 0.0, "deg": 90.0, "out": list(rotate_point(1.0, 0.0, 90.0))},
                {"x": 3.0, "y": 4.0, "deg": 45.0, "out": list(rotate_point(3.0, 4.0, 45.0))},
            ]
        },
        "horizon_clip": {
        ...
        "labels": {
            "reference_rle": rle_u8(labels),
            "perturbed_rle": rle_u8(labels_pert),
        },
        "angle_stack": {
            "shape": [ni, nj, ns],
            "formula": "0.1*i+0.05*j+0.02*k",
            "pert_delta": 1e-4,
        },
    }
